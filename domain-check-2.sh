...
##################################################################
# Purpose: Return whois server
# Arguments:
#   $1 -> TLD
##################################################################
whois_server()
{
     # associate array... boring
#    declare -A SERVERS
#    SERVERS['org']="whois.pir.org"
#    SERVERS['com']=$WHOIS_SERVER
#    SERVERS['net']=$WHOIS_SERVER
#    if [ ${SERVERS[$1]+_} ]; then echo ${SERVERS[$1]}; else echo $WHOIS_SERVER; fi

     # Not reliable
#    echo $1.whois-servers.net

    # Query IANA
    whois -h whois.iana.org $1 | grep whois | awk '/whois:/ { print $2}'
}
...
check_domain_status()
{
    ...
    # Find out whois server
    SERVER=$(whois_server ${TLDTYPE} )

    # Invoke whois to find the domain registrar and expiration date
    ${WHOIS} -h $SERVER "${1}" > ${WHOIS_TMP}
    ...
    then
	REGISTRAR=`cat ${WHOIS_TMP} | ${AWK} -F: '/Organization:/ && $0 != "" { REGISTRAR=substr($0,21,25) } END { print REGISTRAR }'`
    ...
    then
	REGISTRAR=`cat ${WHOIS_TMP} | ${AWK} -F: '/role:/ && $2 != "" { REGISTRAR=substr($2,10,17) } END { print REGISTRAR }'`
    ...
    then
           tdomdate=`cat ${WHOIS_TMP} | ${AWK} '/expires:/ { print $4"-"$2"-"$3 }'`
           tyear=`echo ${tdomdate} | ${CUT} -d'-' -f1`
           tmon=`echo ${tdomdate} |${CUT} -d'-' -f2`
	       case ${tmon} in
	             January|01) tmonth=jan ;;
	             February|02) tmonth=feb ;;
	             March|03) tmonth=mar ;;
	             April|04) tmonth=apr ;;
	             May|05) tmonth=may ;;
	             June|06) tmonth=jun ;;
	             July|07) tmonth=jul ;;
	             August|08) tmonth=aug ;;
	             September|09) tmonth=sep ;;
	             October|10) tmonth=oct ;;
	             November|11) tmonth=nov ;;
	             December|12) tmonth=dec ;;
	             *) tmonth=0 ;;
	       esac
	   tday=`echo ${tdomdate} | ${CUT} -d "-" -f 3 | ${CUT} -d "T" -f 1`
           DOMAINDATE=`echo "${tday}-${tmonth}-${tyear}"`
    elif [ "${TLDTYPE}" == "com" -o "${TLDTYPE}" == "net" -o "${TLDTYPE}" == "org"  -o "${TLDTYPE}" == "link" -o "${TLDTYPE}" == "blog" -o "${TLDTYPE}" == "tech" -o "${TLDTYPE}" == "io" -o "${TLDTYPE}" == "cloud" -o "${TLDTYPE}" == "cafe" ]; # added on 26-aug-2017 by nixCraft {https://www.cyberciti.biz/}
    then
...
#           tdomdate=`cat ${WHOIS_TMP} | ${AWK} '/Registry Expiry Date:/ { print $NF }'`
	...
	echo "Il dominio ${DOMAIN} scadra' il ${DOMAINDATE} - Mancano ${DOMAINDIFF} giorni (${REGISTRAR})\n"
...
prints()
{
    ...
    then
            ...
            printf -v MSGTEMP "%-35s %-46s %-8s %-11s %-5s\n" "$1" "$5" "$2" "$MIN_DATE" "$4"
	    MSG="$MSG$MSGTEMP"
    fi
}
...
then
	MSG=""
        while read DOMAIN
        do
                MSG+=$(check_domain_status "${DOMAIN}")
